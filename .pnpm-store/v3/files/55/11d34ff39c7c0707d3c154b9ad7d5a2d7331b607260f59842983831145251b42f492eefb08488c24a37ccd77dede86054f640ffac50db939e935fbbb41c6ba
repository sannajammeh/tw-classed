...
async function version(cwd, options, config) {
  ...
  let releasePlan = assembleReleasePlan(changesets, packages, releaseConfig, preState, options.snapshot ? {
    ...
    commit: ((_config$snapshot$prer = config.snapshot.prereleaseTemplate) === null || _config$snapshot$prer === void 0 ? void 0 : _config$snapshot$prer.includes("{commit}")) ? await getCurrentCommitId({
      cwd
    }) : undefined
  } : undefined);
