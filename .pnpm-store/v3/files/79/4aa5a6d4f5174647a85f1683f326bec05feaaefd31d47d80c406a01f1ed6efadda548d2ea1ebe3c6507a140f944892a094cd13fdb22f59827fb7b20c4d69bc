...
async function version(cwd, options, config) {
  ...
  let releasePlan = assembleReleasePlan__default['default'](changesets, packages, releaseConfig, preState, options.snapshot ? {
    ...
    commit: ((_config$snapshot$prer = config.snapshot.prereleaseTemplate) === null || _config$snapshot$prer === void 0 ? void 0 : _config$snapshot$prer.includes("{commit}")) ? await git.getCurrentCommitId({
      cwd
    }) : undefined
  } : undefined);
