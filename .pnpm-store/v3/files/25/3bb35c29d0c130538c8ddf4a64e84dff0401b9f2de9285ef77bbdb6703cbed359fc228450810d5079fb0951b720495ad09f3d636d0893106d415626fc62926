...
var _chunkNHXVM6VLjs = require('./chunk-NHXVM6VL.js');
...
var _chunkJTK5TDISjs = require('./chunk-JTK5TDIS.js');
  ...
  return {
    ...
    setup({ onResolve }) {
      onResolve({
        ...
        path: path10.slice(nodeProtocol.length),
        external: true
      }));
    }
  };
...
var generateExternal = async (external) => {
  ...
  for (const item of external) {
    ...
    const deps = await _chunkJTK5TDISjs.getProductionDeps.call(void 0, pkgPath);
    result.push(...deps);
  }
  return result;
};
async function runEsbuild(options, {
  ...
  const pkg = await _chunkJTK5TDISjs.loadPkg.call(void 0, process.cwd());
  const deps = await _chunkJTK5TDISjs.getProductionDeps.call(void 0, process.cwd());
  const external = [
  ...
  if (result && result.warnings && !_chunkJTK5TDISjs.getSilent.call(void 0, )) {
...
var isJS = (path10) => /\.(js|mjs|cjs)$/.test(path10);
var isCSS = (path10) => /\.css$/.test(path10);
...
var sizeReporter = () => {
  return {
    ...
    buildEnd({ writtenFiles }) {
      _chunkJTK5TDISjs.reportSize.call(void 0, 
        this.logger,
        this.format,
...
var import_rollup_plugin_hashbang = _chunkEBJAWNYGjs.__toESM.call(void 0, _chunkJTK5TDISjs.require_dist.call(void 0, ));
...
var normalizeOptions = async (logger, optionsFromConfigFile, optionsOverride) => {
  ...
  _chunkJTK5TDISjs.setSilent.call(void 0, options.silent);
...
async function build(_options) {
  const config = _options.config === false ? {} : await _chunkJTK5TDISjs.loadTsupConfig.call(void 0, 
    process.cwd(),
  ...
  await Promise.all(
    [...Array.isArray(configData) ? configData : [configData]].map(
      async (item) => {
        const logger = _chunkJTK5TDISjs.createLogger.call(void 0, item == null ? void 0 : item.name);
        const options = await normalizeOptions(logger, item, _options);
        logger.info("CLI", `tsup v${_chunkNHXVM6VLjs.version}`);
        if (config.path) {
        ...
        const mainTasks = async () => {
          ...
          if (!((_a = options.dts) == null ? void 0 : _a.only)) {
            ...
            let depsHash = await _chunkJTK5TDISjs.getAllDepsHash.call(void 0, process.cwd());
              ...
              watcher.on("all", async (type, file) => {
                ...
                if (options.watch === true) {
                  if (file === "package.json" && !buildDependencies.has(file)) {
                    const currentHash = await _chunkJTK5TDISjs.getAllDepsHash.call(void 0, process.cwd());
